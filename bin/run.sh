...
if [ $1 = "android" ]; then
    docker run --rm --mac-address=12:34:56:78:9a:bc -ti -p 3000:3000 -p 8005:8005 -p 53703:53703 -v $(pwd)/src:/app/src -v $(pwd)/www:/app/www --privileged -v /dev/bus/usb:/dev/bus/usb theomathieu/frontend-dev $1 $(ifconfig | grep '172.16' | tail -1 | cut -d ':' -f 2 | cut -d ' ' -f 1)
fi
